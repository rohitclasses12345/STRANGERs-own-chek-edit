...
async def process_audio(client, event, url, cookies_env_var=None):
    ...
    random_filename = f"@Strangerboys24_{event.sender_id}"
    ...
    try:
        ...
        if os.path.exists(download_path):
            def edit_metadata():
                ...
                audio_file.tags["TPE1"] = TPE1(encoding=3, text="STRANGER ")
                audio_file.tags["COMM"] = COMM(encoding=3, lang="eng", desc="Comment", text="Processed by STRANGER ")
...
async def process_video(client, event, url, cookies_env_var, check_duration_and_size=False):
    ...
    try:
        ...
        title = info_dict.get('title', 'Powered by STRANGER ')
